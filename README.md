...
```.cpp
	...
	//presetsManager.setGroupLinkSize(2);// define how many presets we want for the Group_Link selector. default amount is 10 presets.
	...
	{
		ofLogNotice(__FUNCTION__) << "DONE LOAD PRESET. SOME GROUP HAVE CURRENTLY LOADED/CHANGED TO ANOTHER PRESET INDEX";
	}

	// B. better callbacks for all selectors, to know when a group selector changed:
	// wich group and wich presets index selector changed.
	// ofApp::setup()
	...
	void ofApp::Changed_PresetsManagerSelectors(ofAbstractParameter &e)
	{
		...
		ofLogNotice(__FUNCTION__) << "THE SELECTOR FOR THER GROUP WITH NAME " << name << " CHANGED TO PRESET INDEX: " << e;
		if (name == params.getName()) {
			// we know that some/all ofParameters of this ofParameterGroup changed,
	...
	// also allows to use a custom path folder out of /bin/data/. 
	// ie: to share same preset files folder between different apps. 
	// ofApp::setup()
